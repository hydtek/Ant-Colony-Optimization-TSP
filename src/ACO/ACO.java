...
public class ACO {
    ...
    private void runPass() {
        ...
        for (int j = 0; j < numCities; j++) {
            for (int i = 0; i < popAnts; i++) {
                ...
                if (j == 0) //adds city to next spot in tour
                {
                    ants[i].nextCity(i);
                } //finds best choice to add city to graph
                else {
                    int bestChoice = antIteration(ants[i], j); //best choice to add next city to tour
    ...
    private int antIteration(Ant a, int pos) {
        ...
        for (int i = 0; i < numCities; i++) {
            ...
            if (cityContained(a, i) == false) {
                ...
                double distance = Math.pow((1 / (HeuristicDist(fileX[a.getCity(pos - 1)], fileX[i], fileY[a.getCity(pos - 1)], fileY[i]))), beta);  //distance of each edge
    ...
    private void initialPheromone() {
        ...
        double iniPher = Double.parseDouble(input); //value for user input for pheromone
    ...
    private boolean cityContained(Ant a, int c) {
    ...
    private double HeuristicDist(int x1, int x2, int y1, int y2) {
        //formulate to find distance of two cities
        double score = (int) (Math.sqrt(Math.pow((x1 - x2), 2) + (Math.pow((y1 - y2), 2))));  //score of an edge
        return score;

    }

    //finds the score of an entire ant
    private double score(Ant c) {
    ...
    private double bestScore(Ant[] a) {
        int pos     = 0;           //what position the best score occured

        double best = score(a[0]); //initializing the first ant to be best value
    ...
    private double evaporation(double edge, int x, int y) {
        
        //this formula finds the evaporation needed to take place
        double evapo = ((1 - evapRate) * pheromone[x][y]) + (1 / edge); //double to represent evaporation takin place
    ...
    private void finalReport(double[] s, double[] a) {
